    ...
    CACHE
        .get_or_init(|| {
            let home = crate::store::pick_home(
                std::env::var("ZCODE_POOL_HOME").ok().map(std::path::PathBuf::from),
...
fn normalize_quota_limit(limit_resp: &Value, sub_resp: Option<&Value>) -> QuotaOverview {
    ...
    if let Some(sub) = sub_resp {
        if business_ok(sub) {
            if std::env::var("ZPOOL_DUMP_SUB").is_ok() {
                eprintln!("[zpool] subscription/list raw: {sub}");
            }
...
fn normalize_balance(balance_data: &Value) -> QuotaOverview {
    ...
    if std::env::var("ZPOOL_DUMP_SUB").is_ok() {
        eprintln!("[zpool] billing/balance raw: {balance_data}");
    }
    let mut pool = vec![];
