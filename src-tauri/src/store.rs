...
pub(crate) fn pick_home(override_home: Option<PathBuf>, userprofile: Option<PathBuf>, home_env: Option<PathBuf>) -> PathBuf {
    override_home
        .or(userprofile)
        .or(home_env)
...
impl Paths {
    pub fn detect() -> Paths {
        let home = pick_home(
            std::env::var("ZCODE_POOL_HOME").ok().map(PathBuf::from),
    ...
    pub fn store_dir(&self) -> PathBuf { self.home.join(".zcode-pool") }
...
fn in_sandbox() -> bool {
    std::env::var("ZCODE_POOL_HOME").is_ok()
}
