...
async fn oauth_begin(
    ...
    *pending_oauth_guard() = Some(PendingOAuth {
    ...
    .on_navigation(move |url| {
        ...
        // 真实页面跳转也记一笔：注册页切没切过去、被没被弹回来，一眼可见
...
/// 主窗 -> 邮箱池：把某个注册邮箱的最新激活链接捞回来。
///
/// 直连微软 Graph（`client_id` + `refresh_token` 都在池子里），不依赖任何
...
/// 从 txt 文件导入邮箱池。
///
/// 文件每行：`email----password----client_id----refresh_token`。
/// 已存在的邮箱不覆盖，只计入 skipped。
#[tauri::command]
...
/// 导出邮箱池为账号密码文本（`email----password----client_id----refresh_token`）。
#[tauri::command]
